...
func List(ctx context.Context, uid int64, req *httpapi.ListTimelineHTTPRequest) (*ListResponse, error) {
	...
	logs.CtxInfo(ctx,
		"[deep_agent_sdk] list timeline start: uid=%d dialog_stream_id=%d thread_id=%d turn_id=%q cursor=%q limit=%d backward=%t",
	...
	if _, err := session.RequireView(ctx, uid, req.GetSessionID(), false); err != nil {
		logs.CtxError(ctx, "[deep_agent_sdk] list timeline load session failed: uid=%d dialog_stream_id=%d err=%v", uid, req.GetSessionID(), err)
		return nil, err
	}
	cursor := stringValue(req.Cursor)
	agentAPI, err := deps.AgentAPI()
	if err != nil {
		logs.CtxError(ctx, "[deep_agent_sdk] list timeline create cloudagent api failed: uid=%d dialog_stream_id=%d err=%v", uid, req.GetSessionID(), err)
		return nil, common.Downstream("create cloudagent api", err)
	}
	logs.CtxInfo(ctx, "[deep_agent_sdk] list timeline call cloudagent.ListTimeline: uid=%d dialog_stream_id=%d", uid, req.GetSessionID())
	...
	if err != nil {
		logs.CtxError(ctx, "[deep_agent_sdk] list timeline cloudagent.ListTimeline failed: uid=%d dialog_stream_id=%d err=%v", uid, req.GetSessionID(), err)
		return nil, common.Downstream("cloudagent.ListTimeline", err)
	}
	logs.CtxInfo(ctx,
		"[deep_agent_sdk] list timeline success: uid=%d dialog_stream_id=%d event_count=%d next_cursor=%q has_more=%t",
...
func Subscribe(ctx context.Context, uid int64, req *httpapi.SubscribeTimelineHTTPRequest, w io.Writer) error {
	...
	startedAt := time.Now()
	logs.CtxInfo(ctx,
		"[deep_agent_sdk] subscribe timeline start: uid=%d dialog_stream_id=%d thread_id=%s turn_id=%q recover_queue_id=%q",
		uid, req.GetSessionID(), int64PtrString(req.ThreadID), stringPtrValue(req.TurnID), stringPtrValue(req.RecoverQueueID),
	)
	if _, err := session.RequireView(ctx, uid, req.GetSessionID(), false); err != nil {
		logs.CtxError(ctx, "[deep_agent_sdk] subscribe timeline load session failed: uid=%d dialog_stream_id=%d err=%v", uid, req.GetSessionID(), err)
		return err
	}
	agentAPI, err := deps.AgentAPI()
	if err != nil {
		logs.CtxError(ctx, "[deep_agent_sdk] subscribe timeline create cloudagent api failed: uid=%d dialog_stream_id=%d err=%v", uid, req.GetSessionID(), err)
	...
	err = agentAPI.SubscribeTimeline(ctx, cloudapi.SubscribeTimelineRequest{
		...
		if frame.QueueID != "" {
			frameCount++
			logs.CtxInfo(ctx, "[deep_agent_sdk] subscribe timeline emit queue: uid=%d dialog_stream_id=%d queue_id=%s frame_count=%d",
		...
		if frame.Event != nil {
			frameCount++
			logs.CtxInfo(ctx,
				"[deep_agent_sdk] subscribe timeline emit event: uid=%d dialog_stream_id=%d event_id=%s event_type=%s thread_id=%s turn_id=%s created_at_ms=%d payload_bytes=%d frame_count=%d",
	...
	if err != nil {
		if isExpectedStreamClose(ctx, err) {
			logs.CtxInfo(ctx,
				"[deep_agent_sdk] subscribe timeline closed: uid=%d dialog_stream_id=%d elapsed=%s frame_count=%d err=%v",
				uid, req.GetSessionID(), time.Since(startedAt), frameCount, err,
			)
			return nil
		}
		logs.CtxError(ctx,
			"[deep_agent_sdk] subscribe timeline failed: uid=%d dialog_stream_id=%d elapsed=%s frame_count=%d err=%v",
			uid, req.GetSessionID(), time.Since(startedAt), frameCount, err,
		)
		return err
	}
	logs.CtxInfo(ctx,
		"[deep_agent_sdk] subscribe timeline finished: uid=%d dialog_stream_id=%d elapsed=%s frame_count=%d",
...
func isExpectedStreamClose(ctx context.Context, err error) (expected bool) {
	if err == nil {
		return false
	}
