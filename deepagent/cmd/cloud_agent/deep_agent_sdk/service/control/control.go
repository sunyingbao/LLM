...
func StopRunning(ctx context.Context, uid int64, req *httpapi.StopRunningHTTPRequest) (*httpapi.StopRunningHTTPResponse, error) {
	...
	logs.CtxInfo(ctx, "[deep_agent_sdk] stop running start: uid=%d dialog_stream_id=%d reason=%q", uid, req.GetSessionID(), req.GetReason())
	view, err := session.RequireView(ctx, uid, req.GetSessionID(), true)
	if err != nil {
		logs.CtxError(ctx, "[deep_agent_sdk] stop running load session failed: uid=%d dialog_stream_id=%d err=%v", uid, req.GetSessionID(), err)
		return nil, err
	}
	logs.CtxInfo(ctx, "[deep_agent_sdk] stop running session loaded: uid=%d dialog_stream_id=%d main_thread_id=%d",
		uid, req.GetSessionID(), mainThreadID(view))
	agentAPI, err := deps.AgentAPI()
	if err != nil {
		logs.CtxError(ctx, "[deep_agent_sdk] stop running create cloudagent api failed: uid=%d dialog_stream_id=%d err=%v", uid, req.GetSessionID(), err)
		return nil, common.Downstream("create cloudagent api", err)
	}
	logs.CtxInfo(ctx, "[deep_agent_sdk] stop running call cloudagent.StopRunning: uid=%d dialog_stream_id=%d reason=%q",
	...
	if err != nil {
		logs.CtxError(ctx, "[deep_agent_sdk] stop running cloudagent.StopRunning failed: uid=%d dialog_stream_id=%d err=%v", uid, req.GetSessionID(), err)
		return nil, common.Downstream("cloudagent.StopRunning", err)
	}
	logs.CtxInfo(ctx, "[deep_agent_sdk] stop running success: uid=%d dialog_stream_id=%d control_message_count=%d refs=%v",
