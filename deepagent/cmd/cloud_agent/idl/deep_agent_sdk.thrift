namespace go deep_agent_sdk
namespace py deep_agent_sdk
namespace java com.bytedance.deep_agent_sdk.api

include "base.thrift"
include "deep_agent_sdk_common.thrift"
...
struct UpdateSessionHTTPRequest {
  1: i64 session_id (go.tag = 'json:"session_id,string"', agw.cli_conv="str"),
  4: optional string title,
  6: optional deep_agent_sdk_common.AgentSessionStatus status,
}
...
struct SubmitInputHTTPRequest {
  ...
  6: optional deep_agent_sdk_common.ResumeRef resume_ref,
  7: optional deep_agent_sdk_common.ApprovalInput approval,
  // 不填表示普通输入；IMPL_PLAN 表示用户确认开始执行已有 plan；
  // COMPACT_CONTEXT 表示手动压缩当前 thread 上下文，不进入模型历史。
  8: optional deep_agent_sdk_common.InputMode mode,
}
...
struct ListTimelineHTTPResponse {
  ...
  2: deep_agent_sdk_common.PageInfo page_info,
...
struct CreateSessionHTTPResponse {
  1: optional deep_agent_sdk_common.AgentSessionView session_view,
  255: base.BaseResp BaseResp,
}

struct ListSessionsHTTPResponse {
  1: optional list<deep_agent_sdk_common.AgentSession> sessions,
  2: deep_agent_sdk_common.PageInfo page_info,
  255: base.BaseResp BaseResp,
}
...
struct CloseProjectHTTPResponse {
  1: optional deep_agent_sdk_common.SessionProject project,
...
struct SubmitInputHTTPResponse {
  1: optional deep_agent_sdk_common.MessageRef message,
  2: optional deep_agent_sdk_common.AgentSessionView session_view,
  255: base.BaseResp BaseResp,
}

struct StopRunningHTTPResponse {
  1: optional deep_agent_sdk_common.AgentSessionView session_view,
  2: optional list<deep_agent_sdk_common.MessageRef> control_messages,
  255: base.BaseResp BaseResp,
}
...
service DeepAgentSDKAPIService {
  // 创建一个 agent session，用于空白会话或预创建会话目录项。
  CreateSessionHTTPResponse CreateSession(1: CreateSessionHTTPRequest req) (api.post="/ad/deep_agent_sdk/create_session"),

  // 获取左侧栏 session 列表。
  ListSessionsHTTPResponse ListSessions(1: ListSessionsHTTPRequest req) (api.post="/ad/deep_agent_sdk/list_sessions"),
  ...
  GetSessionHTTPResponse GetSession(1: GetSessionHTTPRequest req) (api.post="/ad/deep_agent_sdk/get_session"),

  // 更新 session 标题、归档状态等目录属性。
  UpdateSessionHTTPResponse UpdateSession(1: UpdateSessionHTTPRequest req) (api.post="/ad/deep_agent_sdk/update_session"),

  // 关闭或归档一个 session。
  CloseSessionHTTPResponse CloseSession(1: CloseSessionHTTPRequest req) (api.post="/ad/deep_agent_sdk/close_session"),

  // 从侧边栏移除一个 project：关闭该 project 下 active sessions，并保留
  // workspace、timeline、history、checkpoint 和 memory。
  CloseProjectHTTPResponse CloseProject(1: CloseProjectHTTPRequest req) (api.post="/ad/deep_agent_sdk/close_project"),

  // 给 session 提交用户输入或控制指令；普通输入没有 main thread 时由后端按需创建。
  SubmitInputHTTPResponse SubmitInput(1: SubmitInputHTTPRequest req) (api.post="/ad/deep_agent_sdk/submit_input"),

  // 停止 session 当前正在执行的 turn。
  StopRunningHTTPResponse StopRunning(1: StopRunningHTTPRequest req) (api.post="/ad/deep_agent_sdk/stop_running"),
  ...
  ListTimelineHTTPResponse ListTimeline(1: ListTimelineHTTPRequest req) (api.post="/ad/deep_agent_sdk/list_timeline"),

  // 订阅 session timeline 实时事件；HTTP 实现通常会把 AC server stream 转成 SSE。
  //
  // deep_agent_sdk 直接请求 Agent Coordinator SubscribeSession，并把 AC Event.Payload
  // 作为 DeepAgent demo 自有 JSON event 协议透传给前端；不经过 deep_agent_sdk_session RPC。
  SubscribeTimelineHTTPResponse SubscribeTimeline(1: SubscribeTimelineHTTPRequest req) (api.post="/ad/deep_agent_sdk/subscribe_timeline", streaming.mode="server"),
}
