...
当前 `aic_agent_sdk_api` Web UI 已经能跑通基础链路：创建 session、发送消息、展示 agent 事件、切换 main/child thread、审批、plan input、stop running。但它还不是一个真正可展示、可长期使用的对话产品。
...
| aic_agent_sdk_api | `CreateSession` 接收 `project_name`，创建 AC thread 时使用 session 的 `project_path` | HTTP 层负责前端体验聚合，不让前端伪造 uid/path |
| aic_agent_sdk_worker | 使用 AC thread profile cwd 作为工具工作目录 | worker 只关心最终 cwd，不关心 project UI |
...
- `aic_agent_sdk_api` 需要知道 `workspace.root`，用于创建 session 时计算 `project_path`。
- `aic_agent_sdk_worker` 也保留 `workspace.root`，用于没有 profile cwd 的 fallback 和路径约束。
- `aic_agent_sdk_session` 不需要理解 root 规则，只存 API 传入的 `project_name/project_path`。
...
这里 `project_path` 由 `aic_agent_sdk_api` 计算后传给 session RPC，不允许前端直接传。
...
`cmd/cloud_agent/aic_agent_sdk_session/dal/session/model.go`
...
## aic_agent_sdk_api 实现改造

### 配置

`aic_agent_sdk_api` 配置增加：
...
这个 resolver 是 `aic_agent_sdk_api` 内部业务代码，不放进 SDK。
...
2. 调 `aic_agent_sdk_session.ListProjects(uid, status=ACTIVE)`。
3. 返回 `aic_agent_sdk_common.SessionProject` 列表。
...
4. 调 `aic_agent_sdk_session.CreateSession(uid, title, project_name, project_path)`。
5. 返回 session view。
...
3. 调 `aic_agent_sdk_session.ListSessions`。
...
```text
project_name
  -> aic_agent_sdk_api Resolve(uid, project_name)
  -> project_path
  -> t_agent_session.project_path
  -> aic_agent_sdk_api CreateThread Profile.Cwd
  -> aic_agent_sdk_worker threadWorkDir
...
- IDL：
  - `aic_agent_sdk_common.AgentSession` 增加 `project_name/project_path`。
  - `aic_agent_sdk_common.SessionProject`。
  - `aic_agent_sdk_session.CreateSession/ListSessions/ListProjects`。
  - `aic_agent_sdk_api.CreateSession/ListSessions/ListProjects`。
- Overpass：
  - 生成 `aic_agent_sdk_session` kitex。
  - 生成 `aic_agent_sdk_api` hertz。
  - 更新 `aic_agent_sdk_api/aic_agent_sdk_session/aic_agent_sdk_worker` 中公共结构依赖。
- DDL：
