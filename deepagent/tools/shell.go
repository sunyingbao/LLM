...
func startShellJob(command, workingDir string) (job *shellJob, completed <-chan struct{}, err error) {
...
func formatExecuteOutput(output string, exitCode int) (formatted string) {
	parts := []string{output}
	if exitCode != 0 {
