...
func buildAbsoluteVirtualPath(toolPath string) (virtualPath string) {
	if strings.HasPrefix(toolPath, "/") {
		return toolPath
	}
	return sandboxpaths.VirtualPathPrefixRepo + "/" + strings.TrimPrefix(toolPath, "/")
}
...
func resolveToolPath(toolPath string, readOnly bool) (virtualPath string, err error) {
	...
	virtualPath = buildAbsoluteVirtualPath(toolPath)
