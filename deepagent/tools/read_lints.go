...
func GetReadLintsTool(sandboxManager sandbox.SandboxManager) (baseTool tool.BaseTool, err error) {
	return utils.InferTool("read_lints", readLintsToolDesc,
		func(ctx context.Context, in readLintsArgs) (output string, err error) {
			_, err = resolveHostSearchRoot(ctx, sandboxManager, "", true)
			if err != nil {
				return "", err
			}
			return readLints(ctx, in.Paths, sandboxManager)
		})
}

func readLints(ctx context.Context, paths []string, sandboxManager sandbox.SandboxManager) (output string, err error) {
	resolvedPaths := paths
	...
	packages, unsupported, err := getLintTargets(resolvedPaths)
	if err != nil {
		return "", err
	}
	if len(packages) == 0 {
...
func getLintTargets(paths []string) (targets []string, unsupportedTargets []string, err error) {
	if len(paths) == 0 {
