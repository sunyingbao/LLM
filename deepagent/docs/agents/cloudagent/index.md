...
| 路径 | 定位 |
| --- | --- |
| `deepagent/cmd/cloud_agent/main.go` | API、Session、Worker 和 Coordinator 的单进程参考 wiring |
| `deepagent/cmd/cloud_agent/deep_agent_sdk` | HTTP/WebUI reference service |
| `deepagent/cmd/cloud_agent/deep_agent_sdk_session` | 默认 session directory 服务 |
| `deepagent/cmd/cloud_agent/idl` | reference service 的 IDL |

这些 `deepagent/cmd/cloud_agent/*` 代码可以参考、部署或 fork，但不承诺作为稳定 library 入口。业务接入时优先复用 `deepagent/cloud/worker` 和 `deepagent/cloud/api`，不要从 `cmd` 目录反向提炼 SDK 合同。

reference service 的 HTTP API 模型由
`deepagent/cmd/cloud_agent/generate_local_code.sh` 生成到 `deep_agent_sdk/hertz_gen`。
HTTP 直接调用进程内 SessionService；Session 使用普通 Go 数据结构和 error，
仅在 HTTP 边界转换对外字段和错误格式。Coordinator 同样是本仓的进程内实现。
内部 Session Kitex 生成代码及其 RPC IDL 已移除；外部 AIInfra 沙箱 RPC 不受影响。
...
业务服务 / 产品接入层
  ...
  - 参考实现：deepagent/cmd/cloud_agent/deep_agent_sdk + deep_agent_sdk_session

deepagent/cloud/worker
...
deepagent/cloud/api
  ...
  - 依赖 Agent Coordinator 抽象接口，不依赖 deep_agent_sdk_session

deepagent/worker/cloud
...
deepagents / deepagent/core/agentthread
...
- `deepagent/cloud/worker` 不负责 HTTP、登录态、session 列表和 WebUI。
- `deepagent/cloud/api` 不负责登录态、session 目录、main thread 绑定和 Hertz response。
- `deep_agent_sdk` 不实现 worker scan/claim，也不理解 worker timeline payload schema；它只把产品 HTTP/session 语义适配到 `deepagent/cloud/api`。
- `deep_agent_sdk_session` 是产品会话目录，不是 Agent runtime 状态机。

## 什么时候用 deepagent/cloud/worker

使用 `deepagent/cloud/worker` 的前提是业务接受默认 CloudAgent 运行模型：
...
这些场景才应考虑直接使用 `deepagent/worker/cloud`。如果只是想获得服务端 Agent 的默认运行能力，不建议绕过 CloudAgent。

## 最小接入面

普通业务优先使用 `deepagent/cloud/worker/bootstrap`。它把配置加载、模型/Fornax、MCP、MySQL/Redis、history/checkpoint、IDGen、memory、thread refs、AC client 和 Worker 启动统一封装，业务入口通常只剩：
...
本地和部署环境使用同一条代码路径，通过 `-conf` 或 `AGENT_WORKER_CONF` 选择 YAML。仓库提供 `cmd/cloud_agent/conf/worker.local.yml` 本地模板；部署环境需要准备自己的配置文件。只有确实需要自行组装基础设施的接入方，才直接使用 `deepagent/cloud/worker` 的两个底层对象：
...
| 对象 | 作用 |
| --- | --- |
| `Config` | 描述 worker/runtime 行为：namespace、并发、workdir、skills、roles、models、prompt |
| `Deps` | 注入 Coordinator、history store、checkpoint store、thread refs、approval store、workdir resolver |

必填项：

- `Config.Host.Namespace`
- `Deps.Coordinator`：宿主进程初始化的 Coordinator 或实现相同接口的依赖
- `Config.Turn.Models`
- `Config.Turn.Roles`
- `Deps.HistoryStore`
...
当前 `deepagent/cloud/worker` 的 public API 仍然以 `Config` 和 `Deps` 暴露接入面。接入和维护时不要把它理解成一份平铺的启动配置，而要按生命周期分成三类：

| 层级 | 解决的问题 | 典型字段 / 依赖 |
| --- | --- | --- |
| worker host | worker 如何使用注入的 Coordinator、扫描和运行 thread | `HostConfig.Namespace`、`Deps.Coordinator`、`Concurrency`、`ScanLimit`、`LeaseMS`、`IdleTimeout` |
...
典型启动形态：`coordinatorConfig` 是宿主提供的 `coordinator.Config`，包含 MySQL DSN 和 Redis 连接设置。初始化一次后，通过 `Deps.Coordinator` 共享给 Worker。

```go
core, err := coordinator.New(ctx, coordinatorConfig)
if err != nil {
    return err
}
...
```text
deepagent/cmd/cloud_agent/main.go
deepagent/cloud/worker/bootstrap/run.go
```
...
| 协议 | 位置 | 说明 |
| --- | --- | --- |
| 用户输入 / resume / compact payload | `deepagent/cloud/protocol/input` | API 写入 AC message，worker 读取并解释 |
| worker timeline payload | `deepagent/cloud/protocol/event` | worker 写入 AC event payload；只定义 payload，不定义 AC event header |
| timeline wire shape | `deepagent/cloud/protocol/timeline` | API/TUI/WebUI 看到的 `AC event envelope + payload` |
| API use case | `deepagent/cloud/api` | 不绑定 Hertz/session 的 submit、timeline、stop 编排 |
| host message / event contract | `agentworker` | worker host 与 runtime 的最小 Go contract |

`deep_agent_sdk` 对 timeline 的职责是输出：
...
推荐做法是：通过 `deepagent/cloud/api.AgentAPI.Submit` 发送用户输入时，把产品侧关联信息放进 `SubmitRequest.Metadata`。
...
```text
deepagent/cloud/api.SubmitRequest.Metadata
  -> AC message metadata
  -> agentworker.Message.Metadata
  -> agentthread.SubmitInput WithInputMeta
  -> agentthread.Event.ConsumedInputsMeta
  -> deepagent/cloud/protocol/event payload.consumed_inputs_meta
```
...
如果业务需要在 worker 内部旁路观察输出，可以配置 `deepagent/cloud/worker.Deps.ThreadOutputObserver`：
...
| 目的 | 代码入口 |
| --- | --- |
| API 提交 input metadata | `deepagent/cloud/api/types.go` 的 `SubmitRequest.Metadata`，以及 `deepagent/cloud/api/agent_api.go` 的 `Submit` |
| AC message 到 worker message | `deepagent/worker/cloud/adapter.go` 的 `messageFromAC` |
| worker message metadata 进入 Agent Thread | `deepagent/worker/thread/thread_runtime.go` 的 `postUserInput` |
| thread 保存 input meta | `deepagent/core/agentthread/thread.go` 的 `WithInputMeta`、`activeRun.consumedInputsMeta` |
| event payload 输出归因字段 | `deepagent/cloud/protocol/event/event.go` 的 `consumed_inputs_meta` 字段，以及 `deepagent/worker/thread/event_mapper.go` |
| worker 内部旁路观察输出 | `deepagent/cloud/worker/config.go` 的 `Deps.ThreadOutputObserver`，以及 `deepagent/worker/thread/output_observer.go` |

## deepagent/cloud/api 和 deep_agent_sdk 的边界

`deepagent/cloud/api.AgentAPI` 抽的是 CloudAgent 运行时 use case，不是 HTTP 框架：

| 方法 | 负责 | 不负责 |
| --- | --- | --- |
| `CreateThread` | 创建 AC thread，编码初始 input payload，记录 parent metadata | 创建/绑定产品 session |
| `Submit` | 编码 input/resume/compact message，选择 SubmitInput 或 ResumeFromBlock | 自动寻找 main thread |
...
| `StopRunning` | 选择显式 thread 或 session 下 RUNNING/BLOCKED thread；RUNNING 走 RequestInputCancel，approval-blocked 走 cancel-turn resume | 关闭 session 或归档 |

`deep_agent_sdk` 保留业务接入层职责：Hertz handler、登录态、session RequireView、main thread 找/建/绑定、TouchSession、BaseResp、SSE 编码和 WebUI 静态资源。

## session 为什么不在 cloudagent 里

`deep_agent_sdk_session` 是产品接入层，不是 Agent runtime 复用层。
...
- 调 `deepagent/cloud/api.AgentAPI` 创建 thread、发送 message、cancel、list/subscribe event。
- 给 WebUI 输出 SSE 和静态资源。

`deep_agent_sdk_session` 负责：
...
- 需要默认体验：部署 `deepagent/cmd/cloud_agent` 单进程，参考其中的 `deep_agent_sdk` HTTP 模块和 `deep_agent_sdk_session` 本地 Session 服务。
- 需要业务定制：实现自己的 API/session，但复用 `deepagent/cloud/api`、`deepagent/cloud/worker` 和 `deepagent/cloud/protocol/*`。
- 需要完全自定义 runtime：直接用 `deepagent/worker/cloud`。
...
不要为了复用内部实现去 import `deepagent/worker/thread` 或修改 builder 内部文件。若这些扩展点不够，先讨论是否应该新增 `Config` / `Deps` 字段；只有业务确实需要完全控制 runtime 时，才退到底层 `deepagent/worker/cloud`。

## 常见误解

- 不要把 `deepagent/cmd/cloud_agent` 当成 SDK public API。它是 reference implementation，用来验证 SDK 边界和提供 dogfood 服务。
- `deepagent/cloud/worker` 不包含业务 HTTP API、WebUI、session 目录或权限模型；这些属于业务产品层。
- `deepagent/cloud/api` 是不绑定 Hertz/session 的运行时 use case，不负责 main thread 查找、登录态、BaseResp 或 SSE 编码。
- timeline payload 是 worker 写入的业务 payload；AC event envelope 承载 event id、type、thread id、turn id 和时间。
- 如果业务不接受默认 DeepAgent thread builder，不要强行改 `deepagent/worker/thread` 内部实现，应先讨论新增扩展点；只有确实要完全自定义 runtime 时，才退到底层 `deepagent/worker/cloud`。

## 验证

改 `deepagent/cloud/worker`、`deepagent/worker/cloud` 或单进程启动 wiring 后，至少执行：

```bash
go test ./deepagent/cloud/protocol/... ./deepagent/cloud/worker/... ./deepagent/worker/...
go test ./deepagent/cmd/cloud_agent/...
```

改 `deep_agent_sdk` 和 shared protocol 后，执行：

```bash
(cd deepagent/cmd/cloud_agent/deep_agent_sdk && go test ./...)
node --check deepagent/cmd/cloud_agent/deep_agent_sdk/webui/static/api.js
```

真实链路验证参考：

- [Worker E2E](../../runbooks/deepagent-worker-e2e.md)
