...
| `WithConfig(cfg)` | 使用完整配置作为构造起点 |
| `WithMaxSteps(n)` | 设置最大执行步数（默认 100） |
| `WithMaxModelCalls(n)` | 限制单次执行的模型调用次数 |
...
| `WithContinueAfterModel(fn)` | 模型无工具调用时决定是否经 `continue` 节点再次调用模型 |
...
可参考仓库根目录的 [文档索引](../docs/README.md) 和 `cmd/deepagent`、`cmd/cloud_agent` 下的参考实现。
