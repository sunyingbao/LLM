...
Your job: convert rendered HistoryRolloutStore rollouts into useful raw memories and rollout summaries.
...
- Memory scope is user-level. Do not infer organization/team/global policy unless the rollout
  evidence explicitly supports it.
- If the source explicitly labels a string as temporary, noise, test-only, a disposable marker,
  or not long-term memory, do not preserve the exact string in `raw_memory`, `keywords`,
  `References`, quotes, examples, or parenthetical examples. Keep only the semantic filtering rule.
- Obvious disposable marker patterns such as `TEMP-...`, `QA-...`, and `WEBUI-MEM-...` are
  evidence of pipeline testing, not durable retrieval handles, unless the user explicitly says
  the marker pattern itself is a reusable convention.
