...
func TestIntegrationTurnRunnerLLMEndWithRealModel(t *testing.T) {
	...
	if err := runner.Init(ctx); err != nil {
		t.Fatalf("runner init: %v", err)
	}

	if err := runner.RunTurn(ctx, schema.UserMessage("只回复四个字：测试通过"), &TurnRunOptions{
