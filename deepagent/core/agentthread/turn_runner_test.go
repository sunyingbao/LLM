...
// 简单 drain：在 RunTurn 返回后，等待一个短暂窗口收集事件
...
func TestTurnRunnerDropsLateEventAfterEventBusClosed(t *testing.T) {
	runner := NewTurnRunner(&TurnRunnerConfig{}, "thread-drop", "turn-drop", NewSimpleTestContextManager(), make(chan Event))
	...
	runner.emitEvent(context.Background(), EventToolEnd, ToolEndPayload{Name: "late_tool"})
}

// 不使用自定义历史存储：TurnRunner 的 MemoryContextManager 在 rs=nil 下会跳过持久化
...
type invokableBaseTool interface {
...
// 构建一个 TurnRunner 与其事件通道；withApproval 控制是否对 counter 需要审批
func buildAgent(t *testing.T, ctrl *gomock.Controller, withApproval bool, cpStore *checkpointer.InMemoryStore) (*TurnRunner, chan Event) {
	t.Helper()
	...
	if withApproval {
		hitl = &deepagents.HITLConfig{NeedApproveTools: map[string]deeptools.NeedApproval{
	...
	ag := NewTurnRunner(cfg, threadID, turnID, cmw, evBus)
...
func buildCustomInterruptAgent(t *testing.T, ctrl *gomock.Controller, cpStore *checkpointer.InMemoryStore) (*TurnRunner, chan Event) {
	t.Helper()
	ctx := context.Background()
	...
	ag := NewTurnRunner(&TurnRunnerConfig{
		ChatModel:       cm,
...
func buildMixedInterruptAgent(t *testing.T, ctrl *gomock.Controller, cpStore *checkpointer.InMemoryStore) (*TurnRunner, chan Event) {
	t.Helper()
	ctx := context.Background()
	...
	ag := NewTurnRunner(&TurnRunnerConfig{
		...
		HITLConfig: &deepagents.HITLConfig{NeedApproveTools: map[string]deeptools.NeedApproval{
			"counter": func(context.Context, *deeptools.ApprovalInfo) bool { return true },
		}},
...
func TestTurnRunner_InitAppliesToolMask(t *testing.T) {
	ctx := context.Background()
...
func TestTurnRunner_PlanMiddlewareEmitsPlanUpdated(t *testing.T) {
	ctx := context.Background()
	...
	mw := ag.buildPlanMiddleware()
	tools, err := mw.Tools(ctx)
	if err != nil {
		t.Fatalf("Tools() error = %v", err)
	}

	var updateTool invokableBaseTool
	for _, candidate := range tools {
		info, err := candidate.Info(ctx)
		if err != nil {
			t.Fatalf("Info() error = %v", err)
		}
		if info.Name == constant.ToolUpdatePlan {
			var ok bool
			updateTool, ok = candidate.(invokableBaseTool)
			if !ok {
				t.Fatalf("update_plan tool is not invokable: %T", candidate)
			}
			break
		}
	}
	...
	out, err := updateTool.InvokableRun(ctx, `{"explanation":"sync progress","plan":[{"step":"Inspect","status":"completed"},{"step":"Implement","status":"in_progress"}]}`)
	if err != nil {
	...
	select {
	case ev := <-bus:
		if ev.Type != EventPlanUpdated {
			t.Fatalf("event type = %s, want %s", ev.Type, EventPlanUpdated)
		}
		if ev.ThreadID != "thread-plan-v2" || ev.TurnID != "turn-plan-v2" {
			t.Fatalf("unexpected event ids: thread=%s turn=%s", ev.ThreadID, ev.TurnID)
		}
		payload, ok := ev.Payload.(PlanUpdatedPayload)
		if !ok {
			t.Fatalf("payload type = %T, want PlanUpdatedPayload", ev.Payload)
		}
	...
	default:
		t.Fatalf("expected plan_updated event")
	}
}

func TestTurnRunner_InitInjectsPlanTool(t *testing.T) {
	ctx := context.Background()
	ctrl := gomock.NewController(t)
	cm := mock_model.NewMockToolCallingChatModel(ctrl)
	cm.EXPECT().WithTools(gomock.Any()).DoAndReturn(
...
func TestTurnRunner_Callbacks_ToolStartWaitsForLLMEndWhenStreamToolCallDisabled(t *testing.T) {
	ctx := context.Background()
	bus := make(chan Event, 32)
	runner := NewTurnRunner(&TurnRunnerConfig{}, "thread-order", "turn-order", NewSimpleTestContextManager(), bus)
	handler := runner.buildCallbacks()
...
func TestTurnRunner_Callbacks_StreamToolCallDoesNotWaitForLLMEnd(t *testing.T) {
	ctx := context.Background()
	bus := make(chan Event, 32)
...
func TestAgentRunner_NormalToolThenDone(t *testing.T) {
	...
	ag, bus := buildAgent(t, ctrl, false, cp)

	ctx := context.Background()
	input := schema.UserMessage("hi")
	opts := &TurnRunOptions{CheckpointID: "thread-agt:r1"}
	if err := ag.RunTurn(ctx, input, opts); err != nil {
...
func TestTurnRunnerCallsTurnCompletedWithPersistedHistory(t *testing.T) {
	ctrl := gomock.NewController(t)
	runner, _ := buildAgent(t, ctrl, false, checkpointer.NewInMemoryStore())
	called := make(chan []*schema.Message, 1)
	runner.cfg.TurnCompleted = func(ctx context.Context, threadID, turnID string, chatModel model.ToolCallingChatModel, history []*schema.Message) {
	...
	if err := runner.RunTurn(context.Background(), schema.UserMessage("remember this"), &TurnRunOptions{CheckpointID: "thread-agt:r1"}); err != nil {
		t.Fatalf("RunTurn() error=%v", err)
	}
	select {
	case history := <-called:
...
func TestAgentRunner_UnknownToolCallReturnsToolResult(t *testing.T) {
	...
	if err := ag.RunTurn(ctx, schema.UserMessage("hi"), &TurnRunOptions{CheckpointID: "thread-unknown-tool:turn-unknown-tool"}); err != nil {
		t.Fatalf("RunTurn error: %v", err)
	}
...
func TestAgentRunner_ApproveInterruptAndResume(t *testing.T) {
	ctrl := gomock.NewController(t)
	cp := checkpointer.NewInMemoryStore()
	ag, bus := buildAgent(t, ctrl, true, cp)

	ctx := context.Background()
	// 首次运行，预期触发审批中断
	err := ag.RunTurn(ctx, schema.UserMessage("start"), &TurnRunOptions{CheckpointID: "thread-agt:r1"})
	if err != nil {
	...
	// 重新构建 TurnRunner 并恢复运行
	ag2, bus2 := buildAgent(t, ctrl, true, cp)

	resume := map[string]any{p.InterruptID: &deeptools.ApprovalResult{Approved: true}}
	if err := ag2.RunTurn(ctx, nil, &TurnRunOptions{CheckpointID: p.CheckpointID, ResumeData: resume, ResumeInterruptIDs: []string{p.InterruptID}}); err != nil {
...
func TestAgentRunner_SingleCustomInterruptUsesInterruptedPayloadInfo(t *testing.T) {
	...
	if err := ag.RunTurn(context.Background(), schema.UserMessage("start"), &TurnRunOptions{CheckpointID: "thread-custom:r1"}); err != nil {
...
func TestAgentRunner_MixedInterruptsEmitInterruptBatchRequested(t *testing.T) {
	...
	if err := ag.RunTurn(context.Background(), schema.UserMessage("start"), &TurnRunOptions{CheckpointID: "thread-batch:r1"}); err != nil {
...
func TestAgentRunner_LLMRequestingCarriesRawCallbackInput(t *testing.T) {
	...
	if err := ag.RunTurn(ctx, schema.UserMessage("hello"), &TurnRunOptions{CheckpointID: "thread-req:r1"}); err != nil {
		t.Fatalf("RunTurn error: %v", err)
	}
...
func TestAgentRunner_StreamToolCall_StreamableToolEvents(t *testing.T) {
	...
	if err := ag.RunTurn(ctx, schema.UserMessage("start"), &TurnRunOptions{CheckpointID: "thread-stream:r1"}); err != nil {
		t.Fatalf("RunTurn error: %v", err)
	}
...
func TestDeepAgent_Callbacks_StreamToolCallEnabled_ToolCallbacksStillFire_ManyConcurrentCalls(t *testing.T) {
	...
	if err := ag.RunTurn(ctx, schema.UserMessage("start"), &TurnRunOptions{CheckpointID: "thread-stream-many:r1"}); err != nil {
		t.Fatalf("RunTurn error: %v", err)
	}
...
func TestAgentRunner_SubAgentEventsCarryChildLoc(t *testing.T) {
	...
	if err := ag.RunTurn(ctx, schema.UserMessage("start"), &TurnRunOptions{CheckpointID: "thread-sub:r1"}); err != nil {
		t.Fatalf("RunTurn error: %v", err)
	}
...
func TestToolEventMiddleware_ResumeNoDuplicateEvents(t *testing.T) {
	...
	ctx := context.Background()
	// 首次运行：工具需审批 → 中断
	err := ag.RunTurn(ctx, schema.UserMessage("do something"), &TurnRunOptions{CheckpointID: "thread-dedup:r1"})
	if err != nil {
	...
	err = ag2.RunTurn(ctx, nil, &TurnRunOptions{
