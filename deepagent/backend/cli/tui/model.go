...
func New(runtime clientruntime.InteractiveRuntime, sessionID string, cfgs ...*config.Config) (model *Model, err error) {
	...
	if runtime.RuntimeKind() == sdkruntime.RuntimeRemote {
		commands = buildSlashCommandsWithoutSkills(cfg)
		if remoteRuntime, ok := runtime.(remoteSessionRuntime); ok {
			session := remoteRuntime.Session()
			if session.ID != "" {
				runtimeName, cwd = remoteSessionIdentity(session)
			} else {
				cwd = "backend session: new"
			}
		}
	} else {
		commands = buildSlashCommands(cfg)
		runManager = runtimeRun.NewManagerWithStore(
...
func (m *Model) Init() (cmd tea.Cmd) {
	...
	if ok && remoteRuntime.Session().ID != "" {
		m.streaming = true
		cmd = tea.Batch(cmd, followRemoteSessionCmd(remoteRuntime, ""))
	}
	return cmd
}
...
func detachStream(m *Model) {
	if m.detach != nil {
		m.detach()
	}
	m.detach = nil
	if m.rt != nil && m.rt.RuntimeKind() == sdkruntime.RuntimeRemote {
		m.streaming = false
	}
}
