...
func renderQuestionPrompt(req questionRequest, width int) (rendered string) {
	if req.current < 0 || req.current >= len(req.questions) {
		return ""
	}
	question := req.questions[req.current]
	header := strings.TrimSpace(question.header)
	if header == "" {
		header = fmt.Sprintf("Question %d/%d", req.current+1, len(req.questions))
	}
	lines := []string{approvalHeaderStyle.Render("● " + header), question.question}
	for index, option := range question.options {
		lines = append(lines, fmt.Sprintf("  %d. %s", index+1, option))
	}
	lines = append(lines, approvalHintStyle.Render("type an answer and press Enter · Ctrl-C/Esc = stop without answering"))
