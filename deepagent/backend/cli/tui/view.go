...
func (m *Model) View() (view string) {
	...
	if questionPanel := renderQuestionPanel(m); questionPanel != "" {
		sb.WriteString("\n")
		sb.WriteString(questionPanel)
	}
	if questionPanel := renderQuestionPanel(m); questionPanel != "" {
		sb.WriteString("\n")
		sb.WriteString(questionPanel)
...
func renderQuestionPanel(m *Model) (panel string) {
	if len(m.questionQueue) == 0 {
		return ""
	}
	return renderQuestionPrompt(m.questionQueue[0], m.width)
}
