...
var builtinCommands = []slashCommand{
	...
	{Name: "resume", Args: "<session_id>", Desc: "open and follow a backend session (remote only)", Type: "builtin"},
	{Name: "sessions", Desc: "list backend sessions (remote only)", Type: "builtin"},
	{Name: "todos", Args: "[open|close|toggle]", Desc: "expand / collapse the todo panel", Type: "builtin"},
}

var commands = builtinCommands

func buildSlashCommands(cfg *config.Config) (commands []slashCommand) {
	commands = buildSlashCommandsWithSkills(cfg, true)
	return commands
}

func buildSlashCommandsWithoutSkills(cfg *config.Config) (commands []slashCommand) {
	commands = buildSlashCommandsWithSkills(cfg, false)
	return commands
}

func buildSlashCommandsWithSkills(cfg *config.Config, includeSkills bool) (commands []slashCommand) {
...
func attachBuiltinHandlers(commands []slashCommand) {
	for i := range commands {
		...
		case "resume":
			commands[i].Handler = handleResumeCommand
		case "sessions":
			commands[i].Handler = handleSessionsCommand
		case "todos":
...
func handleExitCommand(model *Model, _ string) (cmd tea.Cmd) {
	detachStream(model)
	return tea.Quit
}

func handleClearCommand(m *Model, _ string) (cmd tea.Cmd) {
	detachStream(m)
	resetConversationView(m)
	m.rt.ClearThread()
	if m.rt.RuntimeKind() == sdkruntime.RuntimeRemote {
		m.cwd = "backend session: new"
		m.modelName = runtimeDisplayName(m.rt.Name(), m.rt.RuntimeKind())
		resetConversationView(m)
	}
	return nil
}
...
func handleHistoryCommand(m *Model, _ string) (cmd tea.Cmd) {
	if remoteRuntime, ok := m.rt.(remoteSessionRuntime); ok {
		if remoteRuntime.Session().ID == "" {
			pushMessage(m, "system", "history: no backend session selected")
			return nil
		}
		m.streaming = true
		m.lastErr = nil
		return followRemoteSessionCmd(remoteRuntime, "")
	}
...
func handleSessionsCommand(m *Model, _ string) (cmd tea.Cmd) {
	remoteRuntime, ok := m.rt.(remoteSessionRuntime)
	if !ok {
		pushMessage(m, "system", "sessions: available only in remote runtime")
		return nil
	}
	m.streaming = true
	m.lastErr = nil
	return listRemoteSessionsCmd(remoteRuntime)
}

func handleResumeCommand(m *Model, text string) (cmd tea.Cmd) {
	remoteRuntime, ok := m.rt.(remoteSessionRuntime)
	if !ok {
		pushMessage(m, "system", "resume: available only in remote runtime")
		return nil
	}
	sessionID := strings.TrimSpace(strings.TrimPrefix(text, "/resume"))
	if sessionID == "" {
		pushMessage(m, "system", "usage: /resume <session_id>")
		return nil
	}
	detachStream(m)
	m.streaming = true
	m.lastErr = nil
	return followRemoteSessionCmd(remoteRuntime, sessionID)
}
