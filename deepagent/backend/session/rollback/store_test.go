...
func TestStoreRestoreWorkspacePostDoesNotRestoreAgentCheckpoint(t *testing.T) {
	root := t.TempDir()
	...
	if _, err := store.SaveWorkspacePost(context.Background(), "run-1"); err != nil {
		t.Fatalf("SaveWorkspacePost() error = %v", err)
	}

	writeTestFile(t, root, ".eino-cli/sessions/"+sid+"/checkpoints/state.json", "checkpoint-2")
	writeTestFile(t, root, ".eino-cli/sessions/"+sid+"/workspace/data.txt", "workspace-2")
	...
	if err := store.RestoreWorkspacePost(context.Background(), "run-1"); err != nil {
		t.Fatalf("RestoreWorkspacePost() error = %v", err)
	}
	assertTestFile(t, root, ".eino-cli/sessions/"+sid+"/checkpoints/state.json", "checkpoint-2")
	assertTestFile(t, root, ".eino-cli/sessions/"+sid+"/workspace/data.txt", "workspace-1")
