...
func TestRuntimeUserInputExposesThreadAndTurnInfo(t *testing.T) {
	...
	deepThread := agentthread.New("thread-info", &agentthread.TurnRunnerConfig{
		ChatModel:       model,
	...
	runtime, err := NewRuntime(AdapterConfig{
		...
		TurnRunnerConfig: func(ctx context.Context, req TurnRunnerConfigRequest) (*agentthread.TurnRunnerConfig, error) {
...
func TestRuntimeResumeExposesResumeTurnInfo(t *testing.T) {
	...
	eventBus := make(chan agentthread.Event, 16)
	deepThread := agentthread.New("thread-resume-info", &agentthread.TurnRunnerConfig{
		ChatModel:       model,
...
func TestRuntimeInitExposesThreadInfo(t *testing.T) {
	...
	deepThread := agentthread.New("thread-init", &agentthread.TurnRunnerConfig{
		CheckpointStore: checkpointer.NewInMemoryStore(),
	}, cm, eventBus)
	runtime, err := NewRuntime(AdapterConfig{
...
func TestNewRuntimeDefaultsThreadInfoIDs(t *testing.T) {
	...
	deepThread := agentthread.New("thread-default", &agentthread.TurnRunnerConfig{
		CheckpointStore: checkpointer.NewInMemoryStore(),
