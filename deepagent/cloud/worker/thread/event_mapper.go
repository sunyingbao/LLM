...
func agentEventPayloadForOutput(ev agentthread.Event, usage *agentthread.ContextUsageSnapshot) (eventType protoevent.EventType, payload any, err error) {
	defer func() {
		if err == nil && payload != nil {
			attachConsumedMessageIDs(payload, ev.ConsumedInputs)
			err = attachConsumedInputsMeta(payload, ev.ConsumedInputsMeta)
		}
	}()
...
func attachConsumedMessageIDs(payload any, inputs []*schema.Message) {
...
func attachConsumedInputsMeta(payload any, meta []any) error {
