...
func TestRuntimeCallsTurnFinishedObserverOnTurnEnd(t *testing.T) {
	...
	cfg := &agentthread.TurnRunnerConfig{CheckpointStore: checkpointer.NewInMemoryStore()}
	deepThread := agentthread.New("thread-1", cfg, newRuntimeCompactContextManager(), events)
...
func TestUserInputMessageBuildsPlainTextContent(t *testing.T) {
	got, err := userMessageToEinoMessage(protoinput.UserMessage{
		Parts: []protoinput.MessagePart{
...
func TestRunnerConfigMarksSubmitAndResume(t *testing.T) {
	ctx := context.Background()
	var requests []TurnRunnerConfigRequest
	runtime := &Runtime{
		turnRunnerConfig: func(_ context.Context, req TurnRunnerConfigRequest) (*agentthread.TurnRunnerConfig, error) {
			requests = append(requests, req)
			return &agentthread.TurnRunnerConfig{MaxSteps: 99}, nil
		},
	}

	submitMsg := &agentworker.Message{ID: "m-submit"}
	got, err := runtime.runnerConfig(ctx, "turn-submit", submitMsg, protoinput.UserMessageModeImplPlan, false)
	if err != nil {
		t.Fatal(err)
	}
	if got == nil || got.MaxSteps != 99 {
		t.Fatalf("submit runner config=%+v, want MaxSteps=99", got)
	}

	resumeMsg := &agentworker.Message{ID: "m-resume"}
	got, err = runtime.runnerConfig(ctx, "turn-resume", resumeMsg, "", true)
	if err != nil {
		t.Fatal(err)
	}
	if got == nil || got.MaxSteps != 99 {
...
func TestRuntimePostResumeTurnResolvesConfigWithoutStoredSnapshot(t *testing.T) {
	...
	events := make(chan agentthread.Event, 16)
	deepThread := agentthread.New("thread-reclaim", &agentthread.TurnRunnerConfig{
		ChatModel:       model,
...
func TestRuntimePostMessageBuildsTurnRunnerConfigOnlyForNewTurn(t *testing.T) {
	ctx := context.Background()
	...
	cm := newRuntimeCompactContextManager()
	thread := agentthread.New("1", &agentthread.TurnRunnerConfig{
		ChatModel:       model,
...
func newTestCompactRuntime(t *testing.T, cm *runtimeCompactContextManager, model modelcomp.ToolCallingChatModel) (*Runtime, *agentworker.ThreadOutput) {
	...
	thread := agentthread.New("1", cfg, cm, eventBus)
	runtime, err := NewRuntime(AdapterConfig{
		SessionID: "100",
		ThreadID:  "1",
		Thread:    thread,
