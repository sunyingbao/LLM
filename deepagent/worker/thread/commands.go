...
func decodeUserInputCommand(message *agentworker.Message) (cmd userInputCommand, err error) {
	input, err := parseUserMessage(message)
	if err != nil {
		return userInputCommand{}, err
	}
	userInput, err := cloudUserMessageToSchemaMessage(input)
	if err != nil {
		return userInputCommand{}, err
	}
...
func decodeResumeTurnCommand(message *agentworker.Message) (cmd resumeTurnCommand, err error) {
...
func workerMessageID(message *agentworker.Message) (id string) {
	if message == nil {
		return ""
	}
	return message.ID
