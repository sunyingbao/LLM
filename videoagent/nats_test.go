...
func TestNATSMessageBusRetriesAndDeduplicatesCallbacks(t *testing.T) {
	...
	defer bus.Close()
	defer bus.jetStream.DeleteStream(context.Background(), config.Stream)
...
func TestNATSMessageSurvivesClientRestart(t *testing.T) {
	...
	defer consumer.jetStream.DeleteStream(context.Background(), config.Stream)
	consumeContext, cancel := context.WithCancel(context.Background())
	consumerDone := make(chan error, 1)
	...
	for time.Now().Before(deadline) {
		info, infoErr := consumer.consumer.Info(context.Background())
...
func TestNATSMessageMovesToDeadLetter(t *testing.T) {
	...
	stream, err := bus.jetStream.Stream(context.Background(), config.Stream)
	if err != nil {
...
func TestNATSApplicationResumesAfterRestart(t *testing.T) {
	...
	defer secondBus.jetStream.DeleteStream(context.Background(), config.Stream)
