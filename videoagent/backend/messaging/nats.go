...
func (bus *NATSMessageBus) consumeMessages(ctx context.Context, handle func(context.Context, CallbackMessage) error) error {
	...
	for {
		...
		if handleErr != nil {
			...
			if !isPollingMessage(callback) && !errors.Is(handleErr, contract.ErrJobPending) && metadataErr == nil && metadata.NumDelivered >= bus.maxRetries {
...
func (bus *NATSMessageBus) DeleteStream(ctx context.Context, name string) error {
	if bus == nil || bus.jetStream == nil {
		return fmt.Errorf("nats message bus is nil")
	}
	return bus.jetStream.DeleteStream(ctx, name)
}

func (bus *NATSMessageBus) ConsumerInfo(ctx context.Context) (*jetstream.ConsumerInfo, error) {
	if bus == nil || bus.consumer == nil {
		return nil, fmt.Errorf("nats message bus is nil")
	}
	return bus.consumer.Info(ctx)
}

func (bus *NATSMessageBus) Stream(ctx context.Context, name string) (jetstream.Stream, error) {
	if bus == nil || bus.jetStream == nil {
		return nil, fmt.Errorf("nats message bus is nil")
	}
	return bus.jetStream.Stream(ctx, name)
}
