...
func NewModelPlanner(chatModel model.BaseChatModel) (*ModelPlanner, error) {
	if chatModel == nil {
		return nil, fmt.Errorf("planner chat model is nil")
	}
	return &ModelPlanner{model: chatModel}, nil
}
...
func (planner *ModelPlanner) AnalyzeRequirement(ctx context.Context, input RunInput) (Requirement, error) {
	content, err := planner.generateContent(ctx, "分析商品与用户需求，输出可直接展示的 Markdown 需求分析。", input)
	if err != nil {
...
func (planner *ModelPlanner) CreateClipScript(ctx context.Context, requirement Requirement, input RunInput) (ClipScript, error) {
	content, err := planner.generateContent(ctx, "根据需求生成创意方案和分镜脚本。回答必须依次包含“1. 创意方案如下”和“2. 分镜脚本如下”两个 JSON 区段。", struct {
...
func (planner *ModelPlanner) PlanCompetition(ctx context.Context, script ClipScript, input RunInput) ([]ResourcePlan, error) {
	...
	err := planner.generate(ctx, "为分镜脚本规划竞品参考图任务，返回 JSON 数组；每项包含 id、scene_id、prompt、model、width、height，scene_id 必须来自输入分镜。不要输出 Markdown。", struct {
...
func (planner *ModelPlanner) PlanTTS(ctx context.Context, script ClipScript) ([]ResourcePlan, error) {
	var output []ResourcePlan
	err := planner.generate(ctx, "为每个分镜规划配音任务，返回 JSON 数组；每项包含 id、scene_id、prompt、speaker、text、cpm，scene_id 必须来自输入分镜，prompt 描述音色和说话风格。不要输出 Markdown。", script, &output)
	return output, err
}
...
func (planner *ModelPlanner) PlanCharacterReferences(ctx context.Context, script ClipScript, input RunInput) ([]ResourcePlan, error) {
	var output []ResourcePlan
	err := planner.generate(ctx, "为分镜脚本规划人物参考图任务，返回 JSON 数组；每项包含 id、scene_id、prompt、model、fallback_model、width、height，scene_id 必须来自输入分镜。不要输出 Markdown。", struct {
		Script ClipScript `json:"clipscript"`
		Input  RunInput   `json:"input"`
	}{script, input}, &output)
	return output, err
...
func (planner *ModelPlanner) generate(ctx context.Context, instruction string, input any, output any) error {
	content, err := planner.generateContent(ctx, instruction, input)
	if err != nil {
		return err
	}
...
func (planner *ModelPlanner) generateContent(ctx context.Context, instruction string, input any) (string, error) {
	payload, err := json.Marshal(input)
	if err != nil {
		return "", err
	}
	message, err := planner.model.Generate(ctx, []*schema.Message{
