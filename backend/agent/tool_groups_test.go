...
func TestApplyToolGroups(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			profile: &AgentProfile{Name: "x", ToolGroups: nil},
			want:    want{hasBackend: true, hasShell: true},
		},
		{
			name:    "empty tool_groups disables both",
			profile: &AgentProfile{Name: "x", ToolGroups: []string{}},
			want:    want{hasBackend: false, hasShell: false},
		},
		{
			name:    "filesystem only",
			profile: &AgentProfile{Name: "x", ToolGroups: []string{"filesystem"}},
			want:    want{hasBackend: true, hasShell: false},
		},
		{
			name:    "shell only",
			profile: &AgentProfile{Name: "x", ToolGroups: []string{"shell"}},
			want:    want{hasBackend: false, hasShell: true},
		},
		{
			name:    "both groups, with whitespace + casing",
			profile: &AgentProfile{Name: "x", ToolGroups: []string{"  Filesystem ", "BASH"}},
			want:    want{hasBackend: true, hasShell: true},
		},
		{
			name:    "unknown groups silently dropped",
			profile: &AgentProfile{Name: "x", ToolGroups: []string{"web_search", "mcp"}},
			want:    want{hasBackend: false, hasShell: false},
		},
		{
			name:    "filesystem + unknown group keeps filesystem",
			profile: &AgentProfile{Name: "x", ToolGroups: []string{"filesystem", "web_search"}},
			want:    want{hasBackend: true, hasShell: false},
		},
	}
